...
class StoreStock extends ChangeNotifier {
  ...
  void _loadStock() async {
    ...
    for (final itemJson in data) {
      // Always start with quantity 0 — the JSON may contain stale values
      // (e.g. Full Fat Milk ships with "quantity": 1) that would incorrectly
      // pre-fill the cart on every cold start.
      _stock.add(StoreItem.fromJson(itemJson).copyWith(quantity: 0));
    }
    notifyListeners();
  }
